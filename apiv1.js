...
const ambulancesRoute = "/ambulances"
...
router.get(ambulancesRoute, async (req, res) => {
  const hospitalId = Number(req.query.id); // or req.params.id if you switch to params
console.log({hospitalId});
  try {
    ...
    if (!ambulances.length) {
      return res.status(404).json({ error: "No ambulances found for this hospital" });
    }
